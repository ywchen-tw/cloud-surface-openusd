...
| 6 | Wavy horizontal scanline stripes over the surface (1-3 px period, bluish, in regions seen through the thin cloud veil) | Working hypothesis (two theories refuted first): **denoiser streaking**. Zoomed inspection shows the stripes are IMAGE-space horizontal scanlines, not world-space iso-τ contours — which rules out data/precision/step-size causes. Cycles denoising is ON by default (OpenImageDenoise, HIGH) and smears high-variance input — dark surface behind a scattering veil at 512 samples — into wavy horizontal streaks. Refuted en route: (a) negative-scale mirrored instances — stripes survived the baked-mirror single prim (L40, P100); (b) HALF-precision NanoVDB — stripes survived `precision=FULL` + `clipping=0` (job 27306307, L40). The "GPU-only" pattern was likely confounded: the clean CPU evidence came from 4096-sample nadir renders (low-noise denoiser input), the striped renders were all 256-512-sample oblique hero frames. Note `clipping` default 0.001 WAS still silently discarding β<1e-3 on all devices — the FULL/0 fix stays regardless. | 512-sample hero frames (any device, untested on CPU) | A/B via `--no-denoise` (same frame, same GPU). If confirmed: raise samples so the denoiser input is clean, and/or cut the ultra-thin veil (β<~2e-3) from the hero VDB — it is the variance source AND artifact #4's slabs. |
