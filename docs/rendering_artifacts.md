...
| 6 | Striped/hatched squares over the surface | Cycles GPU volume ray-march banding on **negative-scale** (mirrored) volume instances | CUDA (and presumably OptiX) | Confirmed absent on CPU (same frame). Fix: bake the 3×3 mirror layout into a single pre-mirrored VDB (`cloud_density_mirror3x3.vdb`) — one positive-scale volume prim, seamless by construction. Never mirror volumes by transform. |
