...
- [ ] Extend `tools/generate_week7_vdb.cpp` (or a new `tools/grid_to_vdb.cpp`) to read `data/processed/cloud_ext_64x64x32.f32` + JSON and write `density` fog-volume VDB with anisotropic voxel transform (100 × 100 × 80 m).
- [ ] Point `src/week7_usdvol_cloud.py` (or a new `src/author_cloud_usd.py`) at the real-data VDB; retune domain/extent to the ~6 km tile and `metersPerUnit`.
- [ ] First render of the real LES cloud (visual half of the thesis).
