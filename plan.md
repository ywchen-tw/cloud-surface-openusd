...
- [x] `validation/compare_ear3t.py` — DONE 2026-08-01. EaR3T/MCARaTS (1e9 photons, HG g=0.85, albedo 0.05, SZA 30°/SAA 40° compass) vs Cycles nadir render of the identical cutoff field: **r = 0.988, rel RMSE 13.9%** after (1) CPU render (OptiX zero-radiance artifact), (2) sun-azimuth alignment to er3t's compass convention, (3) 3×3 periodic tiling to match MCARaTS's cyclic BC, (4) 64 volume bounces for the τ~50 cores. Figure + metrics in `$OPENUSD_CLD_DATAROOT/renders/validation/`.
- [x] Render route decided and wired: Blender-Cycles (GPU al40/artxpro6000 for hero frames, CPU amilan for validation frames).
